...
After resolving, `git rebase --continue`, run [Post-fix simplify](#post-fix-simplify) on resolved non-lockfile files, then `git push --force-with-lease`. If `--force-with-lease` is rejected, stop and surface.

## Post-fix simplify

Whenever monitor produces a code edit, run the bundled `simplify` skill on touched files **before** staging — fold any resulting edits into the same commit.
