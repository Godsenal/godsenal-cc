...
- **gbase**: Core productivity tools. Two commands and four skills:
  ...
  - **go** (skill, `disable-model-invocation: true`): One-shot "finish a change" workflow at `plugins/base/skills/go/SKILL.md`. Invokes `/gbase:polish` on the current diff (or latest commit when the tree is clean), then `/gbase:branch-pr`, then `/gbase:monitor` back-to-back. Only triggers on explicit `/gbase:go` — Claude will not auto-run it.
